...
router.get('/members/excel', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=GACIC_Members_${(0, datetime_1.getISTDateString)()}.xlsx`);
...
router.get('/members/pdf', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        doc.fillColor('#666666').fontSize(9).text(`Generated on IST: ${(0, datetime_1.getISTDateString)()} | Total Members: ${members.length}`, { align: 'center' });
        doc.moveDown(1.5);
...
router.get('/attendance/excel', auth_1.authenticateToken, async (req, res) => {
    try {
    ...
    `;
        ...
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', `attachment; filename=GACIC_Sunday_Attendance_${(0, datetime_1.getISTDateString)()}.xlsx`);
        await workbook.xlsx.write(res);
        return res.end();
    }
    catch (error) {
...
router.get('/attendance/pdf', auth_1.authenticateToken, async (req, res) => {
    try {
        const { date } = req.query;
        let sql = `
      SELECT a.service_date, a.check_in_time, a.status, a.scanned_by, m.reg_id, m.full_name, m.mobile_number, m.place_city
    ...
    `;
        ...
        const doc = new pdfkit_1.default({ margin: 30, size: 'A4' });
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename=GACIC_Sunday_Attendance_${(0, datetime_1.getISTDateString)()}.pdf`);
        doc.pipe(res);
        doc.fillColor('#0A192F').fontSize(16).text('Glorious Apostolic Church India Council', { align: 'center' });
        doc.fillColor('#D97706').fontSize(12).text(`Sunday Service Attendance Log ${date ? `(${date})` : ''}`, { align: 'center' });
        doc.fillColor('#666666').fontSize(9).text(`Generated on IST: ${(0, datetime_1.getISTDateString)()} | Total Checked-in: ${records.length}`, { align: 'center' });
        doc.moveDown(1.5);
        let y = doc.y;
        doc.rect(30, y, 535, 20).fill('#0A192F');
