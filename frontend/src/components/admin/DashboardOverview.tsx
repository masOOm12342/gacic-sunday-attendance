          ...
          <div>
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-slate-900">Recent New Visitors</h3>
              </div>
              <button
                onClick={() => onNavigateTab('visitors' as any)}
            ...
            <div className="space-y-3">
              {stats?.recentVisitors && stats.recentVisitors.length > 0 ? (
                stats.recentVisitors.map((v) => (
                  <div key={v.id} className="p-3.5 rounded-2xl bg-emerald-50/50 border border-emerald-100 flex items-center justify-between hover:bg-emerald-100/50 transition-colors">
                    <div className="min-w-0 pr-2">
                      <div className="font-bold text-slate-900 text-sm truncate">{v.full_name}</div>
                      <div className="text-xs text-slate-500 flex items-center gap-1.5 mt-0.5">
                        <span>{v.mobile_number}</span>
                        <span>•</span>
                        <span className="truncate">{v.place_city}</span>
                      </div>
                    </div>
                    <span className="px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-900 font-mono font-bold text-xs shrink-0 border border-emerald-200">
                      {v.visitor_id}
                    </span>
                  </div>
                ))
              ) : (
                <p className="text-xs text-slate-400 text-center py-6">No new visitors registered yet.</p>
              )}
            </div>
          </div>
        </div>
